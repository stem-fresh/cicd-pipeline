...
# Copy the NGINX main configuration file and update pid location to /tmp/nginx.pid
COPY nginx.conf /etc/nginx/nginx.conf

# Copy the server block configuration to /etc/nginx/conf.d/
COPY default.conf /etc/nginx/conf.d/default.conf

# Copy the built application files from the node build stage
COPY --from=node-build /app/build /usr/share/nginx/html

# Expose port 8080 and 8443 for HTTP and HTTPS
EXPOSE 8080 8443
